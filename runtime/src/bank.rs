...
#[allow(deprecated)]
use solana_sdk::{recent_blockhashes_account, recent_evm_blockhashes_account};
use {
    crate::{
...
impl Bank {
    pub fn default_for_tests() -> Self {
        Self::default_with_accounts(Accounts::default_for_tests(), Default::default())
    }
    ...
    fn default_with_accounts(accounts: Accounts, evm_state: EvmState) -> Self {
        let mut bank = Self {
    ...
    pub fn new_with_paths(
        ...
        let evm_state = if let Some((evm_state_path, evm_genesis_path)) = evm_paths {
            evm_state::EvmState::new_from_genesis(
                evm_state_path,
                evm_genesis_path,
                genesis_config.evm_root_hash,
                genesis_config.creation_time as u64,
                false, // spv_compatible never was enabled by default
            )
            .unwrap()
        } else {
    ...
    fn _new_from_parent(
        ...
        let mut new = Bank {
            ...
            evm: EvmBank::new(
                parent.evm.main_chain.chain_id,
                parent
                    .evm
                    .main_chain
                    .evm_blockhashes
        ...
        datapoint_info!(
            ...
            ("evm_state_time", evm_state_time.as_us(), i64),
    ...
    pub(crate) fn get_fields_to_serialize<'a>(
        ...
        BankFieldsToSerialize {
            ...
            evm_chain_id: self.evm.main_chain.chain_id,
            evm_persist_fields: self
                .evm
                .main_chain
                .evm_state
                .read()
    ...
    pub fn evm_block(&self) -> Option<evm_state::Block> {
        self.evm.main_chain.evm_state.read().unwrap().get_block()
    }
    ...
    fn process_genesis_config(&mut self, genesis_config: &GenesisConfig) {
        ...
        let evm_state = Account {
            ...
            data: "Evm state".as_bytes().to_vec(),
            executable: false,
            rent_epoch: 0,
        };
        // Add account for evm.
        let evm_state_account = AccountSharedData::from(evm_state);
        self.store_account_and_update_capitalization(
...
pub(crate) mod tests {
    ...
    const EVM_ACCOUNTS_COUNT: usize = 3; // evm_state, evm_program, evm_blockhashes
    ...
    fn test_evm_really_change_state_in_parallel() {
        ...
        fn test_with_users(num_sleeps: u64) -> (evm_state::H256, evm_state::H256) {
            ...
            let hash_before = bank.evm.main_chain.evm_state.read().unwrap().last_root();
            bank.freeze();
            let hash_after = bank.evm.main_chain.evm_state.read().unwrap().last_root();
    ...
    fn test_evm_revert_tx() {
        ...
        let evm_state = bank.evm.main_chain.evm_state.read().unwrap();
    ...
    fn test_bank_hash_internal_state_verify_transfer_evm() {
        ...
        let init_balance = 10000000.into();
        {
            ...
            let mut evm_state = bank0.evm.main_chain.evm_state.write().unwrap();
            match &mut *evm_state {
        ...
        {
            let evm_state = bank0.evm.main_chain.evm_state.read().unwrap();
            assert_eq!(
        ...
        {
            let evm_state = bank2.evm.main_chain.evm_state.read().unwrap();
            assert_eq!(
                evm_state.get_account_state(sender_addr).unwrap().balance,
        ...
        drop(bank0);
        drop(bank2);
        let evm_state = bank3.evm.main_chain.evm_state.read().unwrap();
        assert_eq!(
            evm_state.get_account_state(sender_addr).unwrap().balance,
            init_balance - 21000 - 1000
        );
    ...
    fn test_bank_transfer_evm_release_lock_panic() {
        ...
        let evm_state = bank2.evm.main_chain.evm_state.read().unwrap().clone();
        {
            assert_eq!(
                evm_state.get_account_state(sender_addr).unwrap().balance,
                init_balance - 21000 - 1000
    ...
    fn test_bank_hash_consistency() {
        ...
        loop {
            ...
            if bank.slot == 0 {
                assert_eq!(
                    bank.hash().to_string(),
                    "J9L2MxfYrahCMrG1i1A4E9XGg69QEcR8qmAfi3JAEYAQ"
                );
            }
            if bank.slot == 32 {
                assert_eq!(
                    bank.hash().to_string(),
                    "DCo9xAMNgvE1jrxdKmZ262LXWVerwdBcUZKT9LATwYhE"
                );
            }
            if bank.slot == 64 {
                assert_eq!(
                    bank.hash().to_string(),
                    "7hMgrPbFDVYPdHxbyJNC26SbRWTDBYw45vqji4n6S8Gh"
                );
            }
            if bank.slot == 128 {
                assert_eq!(
                    bank.hash().to_string(),
                    "49Bmq1mk3X3avzbch3M2DsPv86KVcrwrVDL69DRBQU2Z"
                );
                break;
            }
            bank = Arc::new(new_from_parent(&bank));
    ...
    fn test_shrink_candidate_slots_cached() {
        ...
        assert_eq!(alive_counts, vec![9 + EVM_ACCOUNTS_COUNT, 1, 7]);
    }
    ...
    fn test_process_stale_slot_with_budget() {
        ...
        assert_eq!(consumed_budgets, 10 + EVM_ACCOUNTS_COUNT);
    }
