...
mod internal {
    ...
    pub trait RpcSolPubSubInternal {
        ...
        // Get notification when a new head is set
        ...
        // Unsubscribe from new evm notifications.
        #[rpc(name = "eth_unsubscribe")]
        fn eth_unsubscribe(&self, id: SubscriptionId) -> Result<bool>;
    }
}
    ...
    fn eth_subscribe(
        ...
        match topic.as_ref() {
            ...
            "logs" => {
                ...
                self.subscribe(SubscriptionParams::EvmLogs(filter))
            }
            _ => {
                error!(
